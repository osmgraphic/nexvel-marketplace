...
import {MarketplaceConstants} from "../constants/MarketplaceConstants.sol";
...
contract MarketplaceRegistry is
    Initializable,
    AccessControlUpgradeable,
    UUPSUpgradeable,
    IMarketplaceRegistry
{
    ...
    mapping(
        MarketplaceTypes.MarketplaceModule =>
        MarketplaceTypes.ModuleInfo
    ) private _modules;

    /// @dev Supported NFT collections.
    mapping(
        address =>
        MarketplaceTypes.CollectionConfig
    ) private _collections;

    /// @dev Supported payment tokens.
    mapping(
        address =>
        MarketplaceTypes.PaymentTokenConfig
    ...
    function initialize(
        ...
        if (treasuryAddress == address(0)) {
            revert MarketplaceErrors.InvalidTreasury();
        }

        if (
            marketplaceFeeBps_ >
            MarketplaceConstants.MAX_BPS
        ) {
    ...
    function _setMarketplaceFee(
        ...
        _config.marketplaceFeeBps = marketplaceFeeBps;
    }

    function _setMaxTradeValue(
        uint256 maxTradeValue_
    ) internal {
        if (maxTradeValue == 0) {
            revert MarketplaceErrors.InvalidTradeValue();
        ...
        _config.maxTradeValue = maxTradeValue;
    }
...
function setModule(
    MarketplaceTypes.MarketplaceModule moduleType,
...
{
    return _getModule(
        MarketplaceTypes.MarketplaceModule.NFT721
    );
}

/// @inheritdoc IMarketplaceRegistry
function marketplace1155()
    external
    view
    override
    returns (address)
{
    return _getModule(
        MarketplaceTypes.MarketplaceModule.NFT1155
    );
}
...
{
    return _getModule(
        MarketplaceTypes.MarketplaceModule.PAYMENT
    );
}
    ...
    internal
{
    ...
    MarketplaceTypes.ModuleInfo storage moduleInfo =
        _modules[moduleType];

    if (moduleInfo.module == moduleAddress) {
        return;
    }

    moduleInfo.module = moduleAddress;
    moduleInfo.updatedAt = uint64(block.timestamp);

    emit MarketplaceEvents.ModuleSet(
        moduleType,
        moduleAddress
    );
}
    ...
    internal
{
    ...
    address previousModule = moduleInfo.module;

    delete _modules[moduleType];

    emit MarketplaceEvents.ModuleRemoved(
        moduleType,
        previousModule
    );
}
...
function config()
    external
    view
    returns (MarketplaceTypes.MarketplaceConfig memory)
{
    return _config;
}
...
function collectionConfig(
    ...
    external
    view
    returns (MarketplaceTypes.CollectionConfig memory)
{
...
function paymentTokenConfig(
    address token
)
    external
    view
    returns (MarketplaceTypes.PaymentTokenConfig memory)
{
...
function setCollectionConfig(
    address collection,
    MarketplaceTypes.CollectionConfig calldata config_
)
    external
    onlyRole(REGISTRY_ADMIN_ROLE)
{
...
function setPaymentTokenConfig(
    address token,
    MarketplaceTypes.PaymentTokenConfig calldata config_
)
    external
    onlyRole(REGISTRY_ADMIN_ROLE)
{
    ...
    internal
{
    ...
    MarketplaceTypes.CollectionConfig storage cfg =
    ...
    emit MarketplaceEvents.CollectionConfigUpdated(
        collection,
        config_
    );
}

function _setPaymentTokenConfig(
    address token,
    MarketplaceTypes.PaymentTokenConfig calldata config_
)
    internal
{
    ...
    MarketplaceTypes.PaymentTokenConfig storage cfg =
    ...
    emit MarketplaceEvents.PaymentTokenConfigUpdated(
        token,
        config_
    );
}
