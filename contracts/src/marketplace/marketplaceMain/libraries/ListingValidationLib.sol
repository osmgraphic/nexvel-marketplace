...
library ListingValidationLib {
    ...
    function validateCreateListing(
        MarketplaceTypes.CreateListingParams memory params
    ) internal view {
    ...
    function validatePurchase(
        ...
        ValidationLib.validateExpectedPrice(
            expectedPrice,
            listing.pricePerUnit
        );

        ValidationLib.validateAvailableQuantity(
            quantity,
            listing.quantity
        );
    ...
    /// @param listing Listing.
    function validateActiveListing(
        MarketplaceTypes.Listing memory listing
    ) internal view {
        if (listing.seller == address(0)) {
            revert MarketplaceErrors.ListingNotFound();
