...
library ValidationLib {
    ...
    function validateDeadline(
        ...
        if (
            ...
            revert MarketplaceErrors.DeadlineExpired();
        }
    }
    ...
    function validateERC721Quantity(
        uint256 quantity
    ) internal pure {
        if (quantity != 1) {
            revert MarketplaceErrors.InvalidQuantity();
    ...
    function validateExpectedHighestBid(
        uint256 expected,
        uint256 current
    ) internal pure {
        if (expected != current) {
