...
library RoyaltyLib {
    ...
    function calculateRoyalty(address nft, uint256 tokenId, uint256 salePrice)
        internal
        view
        returns (address receiver, uint256 royaltyAmount)
    {
        if (nft == address(0) || salePrice == 0) {
            return (address(0), 0);
        }
        ...
        (receiver, royaltyAmount) = IERC2981(nft).royaltyInfo(tokenId, salePrice);
    ...
    function supportsERC2981(address nft) internal view returns (bool supported) {
        if (nft == address(0)) {
            return false;
        }

        try IERC165(nft).supportsInterface(_INTERFACE_ID_ERC2981) returns (bool ok) {
            supported = ok;
        } catch {
            supported = false;
        }
    ...
    function hasRoyalty(uint256 royaltyAmount) internal pure returns (bool) {
