    ...
    IMarketplace1155
{
    ...
    function initialize(address admin, address registryAddress)
        external
        initializer
    {
        if (admin == address(0)) {
    ...
    function _assetKey(
        ...
        return keccak256(
            abi.encodePacked(
                nft,
                tokenId,
                MarketplaceTypes.AssetType.ERC1155
            )
        );
    }
    ...
    function getListing(uint256 listingId)
        ...
        view
        override
        returns (MarketplaceTypes.Listing memory)
    {
    ...
    function getAuction(uint256 auctionId)
        ...
        view
        override
        returns (MarketplaceTypes.Auction memory)
    {
        return _auctions[auctionId];
    }

    function listingExists(uint256 listingId)
        external
        view
        returns (bool)
    {
        ...
        returns (uint256 listingId)
    {
        ...
        bytes32 assetKey = _assetKey(
            params.nft,
            params.tokenId
        );

        if (_listingIdByAsset[assetKey] != 0) {
            revert MarketplaceErrors.ListingAlreadyExists(
                params.nft,
                params.tokenId
            );
        }
        ...
        MarketplaceTypes.Listing storage listing =
            _listings[listingId];
        ...
        whenNotPaused
    {
        ...
        if (
            params.newExpiry != 0 &&
            params.newExpiry <= block.timestamp
        ) {
        ...
        emit MarketplaceEvents.ListingUpdated(
            params.listingId,
            params.newPrice,
            params.newExpiry
        );
        ...
        whenNotPaused
    {
        ...
        ListingValidationLib.validatePurchase(
            listing,
            quantity,
            expectedPrice,
            msg.sender
        ...
        uint96 marketplaceFeeBps =
            registryInstance.marketplaceFeeBps();

        uint256 marketplaceFee =
            PaymentLib.calculateMarketplaceFee(
                totalPrice,
                marketplaceFeeBps
            );

        MarketplaceTypes.FeeBreakdown memory breakdown =
            PaymentLib.buildFeeBreakdown(
                totalPrice,
                marketplaceFee,
                royalty.receiver,
                royalty.amount
        ...
        if (PaymentLib.isNativeToken(listing.paymentToken)) {
            PaymentLib.validateNativePayment(
                totalPrice,
                msg.value
            );
        } else {
        ...
        whenNotPaused
    {
        ...
        ListingValidationLib.validateCancelListing(
            listing,
            msg.sender
        );
        ...
        if (
            listing.status !=
            MarketplaceTypes.ListingStatus.ACTIVE
        ) {
            revert MarketplaceErrors.ListingInactive();
        }

        listing.status =
            MarketplaceTypes.ListingStatus.CANCELLED;

        delete _listingIdByAsset[
            _assetKey(
                listing.nft,
                listing.tokenId
            )
        ...
        emit MarketplaceEvents.ListingCancelled(
            listingId,
            listing.nft,
            listing.tokenId,
            listing.seller
        ...
        returns (uint256 auctionId)
    {
        ...
        if (_auctionIdByAsset[assetKey] != 0) {
            revert MarketplaceErrors.AuctionAlreadyExists(
                params.nft,
                params.tokenId
            );
        }
        ...
        MarketplaceTypes.Auction storage auction =
            _auctions[auctionId];
        ...
        auction.status =
            MarketplaceTypes.AuctionStatus.ACTIVE;
    ...
    {
        uint256 amount =
            _pendingRefunds[msg.sender][paymentToken];
        ...
        PaymentLib.refundBid(
            paymentToken,
            msg.sender,
            amount
        );

        emit MarketplaceEvents.RefundWithdrawn(
            msg.sender,
            amount
        );
    }
        ...
        whenNotPaused
    {
        ...
        MarketplaceTypes.BidParams memory params =
            MarketplaceTypes.BidParams({
                auctionId: auctionId,
                amount: amount,
                expectedHighestBid: expectedHighestBid
            });

        AuctionValidationLib.validateBid(
            auction,
            params,
            msg.sender
        );

        PaymentLib.escrowBid(
        ...
        if (
            ...
            auction.endTime - block.timestamp <=
            MarketplaceConstants.AUCTION_EXTENSION_WINDOW
        ) {
            auction.endTime += uint64(
                MarketplaceConstants.AUCTION_EXTENSION_DURATION
            );
        }

        emit MarketplaceEvents.BidPlaced(
            auctionId,
            msg.sender,
            amount,
            auction.endTime
    ...
    function _closeAuction(
        MarketplaceTypes.Auction storage auction,
        MarketplaceTypes.AuctionStatus status
    ) internal {
        ...
        whenNotPaused
    {
        ...
        AuctionValidationLib.validateCancelAuction(
            auction,
            msg.sender
        );
        ...
        _closeAuction(
            auction,
            MarketplaceTypes.AuctionStatus.CANCELLED
        );

        delete _auctionIdByAsset[assetKey];

        IERC1155(auction.nft).safeTransferFrom(
            address(this),
            auction.seller,
            auction.tokenId,
            quantity,
        ...
        emit MarketplaceEvents.AuctionCancelled(
            auctionId,
            auction.seller
        );
    }
        ...
        whenNotPaused
    {
        ...
        AuctionValidationLib.validateSettlement(
            auction
        );

        address treasury =
            registryInstance.treasury();
        ...
        PaymentLib.settleEscrowedAuction(
            auction.paymentToken,
            treasury,
            auction.seller,
            breakdown
        ...
        _closeAuction(
            auction,
            MarketplaceTypes.AuctionStatus.ENDED
        );

        delete _auctionIdByAsset[
            _assetKey(
                auction.nft,
                auction.tokenId
            )
        ...
        emit MarketplaceEvents.AuctionSettled(
            auctionId,
            auction.highestBidder,
            auction.highestBid
        );
        ...
        nonReentrant
    {
        ...
        if (listing.seller == address(0)) {
            revert MarketplaceErrors.ListingNotFound(
                listingId
            );
        }
