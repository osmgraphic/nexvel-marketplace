...
contract NexvelMarketplaceV2 is NexvelMarketplace, IERC721Receiver {
    ...
    event AuctionSettled(
        uint256 indexed auctionId, address indexed nft, uint256 indexed tokenId, address winner, uint256 amount
    );

    event AuctionCancelled(address indexed nft, uint256 indexed tokenId);

    event EmergencyAuctionCancelled(
        uint256 indexed auctionId, address indexed nft, uint256 indexed tokenId, address seller
    );
    ...
    function createAuction(address nft, uint256 tokenId, uint256 minPrice, uint256 duration)
        external
        nonReentrant
        whenNotPaused
        whenGlobalNotPaused
    {
        ...
        emit AuctionCreated(auctionId, nft, tokenId, msg.sender, minPrice, endTime);
    }
    ...
    function bid(address nft, uint256 tokenId) external payable nonReentrant whenNotPaused whenGlobalNotPaused {
    ...
    function settleAuction(address nft, uint256 tokenId) external nonReentrant whenNotPaused whenGlobalNotPaused {
        ...
        if (auction.highestBidder == address(0)) {
            IERC721(nft).safeTransferFrom(address(this), auction.seller, tokenId);
        ...
        IERC721(nft).safeTransferFrom(address(this), auction.highestBidder, tokenId);

        emit AuctionSettled(auction.id, nft, tokenId, auction.highestBidder, auction.highestBid);
    }
    ...
    function cancelAuction(address nft, uint256 tokenId) external nonReentrant {
        Auction memory auction = auctions[nft][tokenId];
    ...
    function emergencyCancelAuction(address nft, uint256 tokenId) external onlyAdmin nonReentrant {
        Auction memory auction = auctions[nft][tokenId];
        ...
        emit EmergencyAuctionCancelled(auction.id, nft, tokenId, auction.seller);
    }
    ...
    function onERC721Received(address, address, uint256, bytes calldata)
        external
        pure
        override(IERC721Receiver, NexvelMarketplace)
        returns (bytes4)
    {
